...
@work_orders_router.post("/", response_model=WorkOrderOut, dependencies=[Depends(roles_allowed("admin", "revisor"))])
async def create_order(data: WorkOrderCreate, db: AsyncSession = Depends(get_db)):
...
async def update_order(order_id: int, data: WorkOrderUpdate, db: AsyncSession = Depends(get_db)):
    service = WorkOrdersService(db)
    return await service.update_work_order(order_id, data)


@work_orders_router.delete("/{order_id}", dependencies=[Depends(roles_allowed("admin"))])
