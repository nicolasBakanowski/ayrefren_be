...
async def create_task(task_in: WorkOrderTaskCreate, db: AsyncSession = Depends(get_db)):
...
@work_order_tasks_router.get("/{work_order_id}", response_model=list[WorkOrderTaskOut],
                             dependencies=[Depends(roles_allowed("admin", "revisor", "mecanico"))])
async def list_tasks(work_order_id: int = Path(..., gt=0), db: AsyncSession = Depends(get_db)):
    service = WorkOrderTasksService(db)
    return await service.list_tasks(work_order_id)


@work_order_tasks_router.delete("/{task_id}", dependencies=[Depends(roles_allowed("admin"))])
